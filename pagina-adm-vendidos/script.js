...
async function inicializarProdutos() {
  ...
  if (tbody) {
    tbody.innerHTML = '<tr><td colspan="5" class="text-center text-primary py-4">Carregando produtos...</td></tr>';
  }
...
async function inicializarPedidos() {
  ...
  if (tbody) {
    tbody.innerHTML = '<tr><td colspan="3" class="text-center text-primary py-4">Carregando pedidos...</td></tr>';
  }
...
function renderizarProdutos(produtos) {
  ...
  // Atualiza contador
  if (contador) {
    contador.textContent = `${produtos.length} produto${produtos.length !== 1 ? 's' : ''} cadastrado${produtos.length !== 1 ? 's' : ''}`;
  }
}
...
function criarLinhaPedido(pedido) {
  const numeroPedido = pedido.cdPedido ? `#${String(pedido.cdPedido).padStart(4, '0')}` : '#0000';
  ...
  if (produtos.length > 0) {
    if (produtos.length === 1) {
      descricaoProdutos = produtos[0].nome || produtos[0].descricao || 'Produto';
    } else {
      descricaoProdutos = `${produtos.length} produtos: ${produtos[0].nome || 'Produto'}${produtos.length > 1 ? ' e outros' : ''}`;
    }
  }
...
function visualizarPedido(id) {
  ...
  const numeroPedido = pedido.cdPedido ? `#${String(pedido.cdPedido).padStart(4, '0')}` : '#0000';
  document.getElementById('modalPedidoLabel').textContent = `Pedido ${numeroPedido}`;
...
function criarConteudoModalPedido(pedido) {
  ...
  if (pedido.cliente?.email || pedido.cdUsuario.emailUsuario) {
    html += `<p class="text-muted small mb-3"><i class="fas fa-envelope me-2"></i>${pedido.cliente?.email || pedido.cdUsuario.emailUsuario}</p>`;
  }
  
  if (pedido.cliente?.telefone || pedido.cdUsuario.nuTelefone) {
    html += `<p class="text-muted small mb-3"><i class="fas fa-phone me-2"></i>${pedido.cliente?.telefone || pedido.cdUsuario.nuTelefone}</p>`;
  }
    ...
    html += `
      <div class="mt-3 pt-3 border-top">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <span class="h5 mb-0 text-primary-purple">R$ ${formatarPreco(total)}</span>
        </div>
      </div>
    `;
  }
...
function criarItemProdutoModal(produto) {
  ...
  const descricao = produto.cdProduto.dsProduto || 'Sem descrição';
  const preco = produto.vlProduto|| 0;
  ...
  return `
    <li class="list-group-item d-flex justify-content-between align-items-start">
      <div class="ms-2 me-auto">
        ...
        <div class="mt-1">
          <span class="text-primary-purple fw-semibold">R$ ${formatarPreco(preco)}</span>
...
function configurarFormularioProduto() {
  ...
  form.addEventListener('submit', async function(e) {
    ...
    // Validação básica
    if (!dadosProduto.nmProduto || !dadosProduto.categoria || !dadosProduto.vlProduto) {
